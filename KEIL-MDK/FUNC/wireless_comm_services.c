...
{
	uint32_t cmd = (uint32_t)self._rx_buf;
...
static void _setGatewayLongAddr(uint8_t* addr)
{
	for(int i=0; i<8;i ++)
	{
		self._gateway_long_addr[i] = addr[i];
	}
}
...
static uint8_t _parseMasterMsg(void)
{
	...
	{
		//检查数据长度是否正确
		if(self._rx_buf[DATA_LEN_BYTE_ID] != self._rx_count)
		{
			_clearFlags();
			return 0;
		}
		...
		if(cmd == CMD_CONNECT_RESP)
		{
			...
			uint64_t gatewayAddr = (uint64_t)&self._rx_buf[DATA_LEN_BYTE_ID+9];
			if(gatewayAddr != (uint64_t)self._gateway_long_addr)
			{
				_clearFlags();
				return 0;
			}
...
wireless_comm_services_t * createWirelessCommServiceHandler(void)
{
	...
	self.setGatewayLongAddr = _setGatewayLongAddr;
