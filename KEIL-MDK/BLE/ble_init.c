...
static void ble_dev_characteristic_mode_addr_update(uint8_t value)
{
	ret_code_t err_code;
	ble_gatts_value_t ble_gatts_value = {
		.len = 1,
...
static void ble_dev_characteristic_interval_addr_update(uint32_t value)
{
	ret_code_t err_code;
	ble_gatts_value_t ble_gatts_value = {
		.len = 4,
...
static ble_characteristic_update_t ble_characteristic_update = {
	...
	.dev_mode_addr_update = ble_dev_characteristic_mode_addr_update,
	.dev_interval_addr_update = ble_dev_characteristic_interval_addr_update,
...
static void gap_params_init(void)
{
	...
	uint8_t mac_addr[6];
	ble_get_address(mac_addr);
	char name_suffix[sizeof(mac_addr)*2+1];
	extern void bytes_to_hex_string(const unsigned char* src, char* dst, int src_len, unsigned char upper_lower);
	bytes_to_hex_string(mac_addr, name_suffix, sizeof(mac_addr), 0);
	name_suffix[4*2] = '\0';
	char dev_name[31];
	sprintf(dev_name, "%s%s", SYS_PARAM_BLE_DEV_NAME, name_suffix);
#endif
...
//	ble_uuid_t adv_uuids[] = {{DEV_CFG_UUID_SERVICE, m_dev_cfg.uuid_type}};
...
__weak void ble_param_tx_power_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_adv_interval_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_adv_time_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_min_conn_interval_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_max_conn_interval_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_slave_latency_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_param_conn_timeout_write_handler(uint8_t* p_data, uint16_t len){}
	
__weak void ble_lora_freq_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_power_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_bw_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_sf_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_code_rate_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_preamble_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_header_write_handler(uint8_t* p_data, uint16_t len){}
__weak void ble_lora_crc_write_handler(uint8_t* p_data, uint16_t len){}
...
ble_characteristic_update_t* ble_characteristic_update_handle_get(void)
{
	return &ble_characteristic_update;
}
...
void BLE_Init(void)
{
	...
	ble_characteristic_update.ble_tx_power_update(param->ble_tx_power);
	ble_characteristic_update.ble_adv_interval_update(param->ble_adv_interval);
	ble_characteristic_update.ble_adv_time_update(param->ble_adv_time);
	ble_characteristic_update.ble_min_conn_interval_update(param->ble_min_conn_interval);
	ble_characteristic_update.ble_max_conn_interval_update(param->ble_max_conn_interval);
	ble_characteristic_update.ble_slave_latency_update(0);
	ble_characteristic_update.ble_conn_timeout_update(param->ble_conn_timeout);
	
	ble_characteristic_update.lora_freq_update(param->lora_freq);
	ble_characteristic_update.lora_power_update(param->lora_power);
	ble_characteristic_update.lora_bw_update(param->lora_bw);
	ble_characteristic_update.lora_sf_update(param->lora_sf);
	ble_characteristic_update.lora_code_rate_update(param->lora_code_rate);
	ble_characteristic_update.lora_preamble_update(param->lora_preamble);
	ble_characteristic_update.lora_header_update(param->lora_header);
	ble_characteristic_update.lora_crc_update(param->lora_crc);
	
	ble_characteristic_update.dev_gateway_addr_update(param->dev_gateway_addr);
	ble_characteristic_update.dev_long_addr_update(param->dev_long_addr);
	ble_characteristic_update.dev_short_addr_update(param->dev_short_addr);
	ble_characteristic_update.dev_mode_addr_update(param->iot_mode);
	ble_characteristic_update.dev_interval_addr_update(param->iot_sample_interval);
	ble_characteristic_update.dev_time_stamp_update(0);
	ble_characteristic_update.dev_battery_update(100);
	ble_characteristic_update.dev_temperature_update(0);
	ble_characteristic_update.dev_x_angle_update(0);
	ble_characteristic_update.dev_y_angle_update(0);
	ble_characteristic_update.dev_x_angle_threshold_update(param->iot_x_angle_threshold);
	ble_characteristic_update.dev_y_angle_threshold_update(param->iot_y_angle_threshold);
	uint8_t sw_version[] = {SYS_SW_MODIFY_VERSION,SYS_SW_MODIFY_VERSION>>8,SYS_SW_SUB_VERSION,SYS_SW_MAIN_VERSION};
	ble_characteristic_update.dev_sw_version_update(sw_version);
	uint8_t hw_version[] = {SYS_HW_MODIFY_VERSION,SYS_HW_MODIFY_VERSION>>8,SYS_HW_SUB_VERSION,SYS_HW_MAIN_VERSION};
	ble_characteristic_update.dev_hw_version_update(hw_version);
}
